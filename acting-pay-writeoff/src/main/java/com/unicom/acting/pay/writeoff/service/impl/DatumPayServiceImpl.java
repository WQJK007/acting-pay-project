...
import com.unicom.acting.pay.domain.ParamInfo;
import com.unicom.acting.pay.domain.RelationInfo;
...
import com.unicom.acting.pay.dao.UserOtherInfoPayDao;
 ...
 */
@Service
public class DatumPayServiceImpl implements DatumPayService {
    private static final Logger logger = LoggerFactory.getLogger(DatumPayServiceImpl.class);
    @Autowired
    private UserOtherInfoPayDao userOtherInfoInDBDao;
    @Autowired
    ...
    public String getOldAcctIdOf2G3G(String userId, String provinceCode) {
        List <ParamInfo> userParamList = getUserParams(userId, "20000060", "ZZZZ", provinceCode);
    ...
    public String getOldUserIdOf2G3G(String userId, String provinceCode) {
        List <ParamInfo> userParamList = getUserParams(userId, "20000050", "ZZZZ", provinceCode);
        if (CollectionUtils.isEmpty(userParamList)) {
...
//        }
    ...
    public String getWJMainSN(String userId, String relationType, String sysdate, String provinceCode) {
        ...
        List<RelationInfo> userRelations = getUserRelation(userId, "0", relationType, "ZZZZ", provinceCode);
        if (CollectionUtils.isEmpty(userRelations)) {
            return "";
        } else {
            String mainUserId = "";
            for (RelationInfo relationInfo : userRelations) {
                //获取当前有效的融合群组关系的虚拟用户标识
                if (relationType.contains("|" + relationInfo.getRelationTypeCode() + "|")
                        && relationInfo.getStartDate().compareTo(sysdate) <= 0
                        && sysdate.compareTo(relationInfo.getEndDate()) <= 0) {
                    mainUserId = relationInfo.getUserId();
                    break;
                }
            }
            List <ParamInfo> userParamList = getUserParams(mainUserId, "20000006", "ZZZZ", provinceCode);
            if (CollectionUtils.isEmpty(userParamList)) {
                return "";
            } else {
                return userParamList.get(0).getParaValue();
    ...
    private List<ParamInfo> getUserParams(String userId, String paramId, String eparchyCode, String provinceCode) {
        ...
        if (!SysTypes.SYS_SUCCESS_CODE.equals(rsp.getRspCode())) {
            ...
            List<ParamInfo> userParamList = JsonUtil.transMapsToObjs(rsp.getData(), ParamInfo.class);
            if (CollectionUtils.isEmpty(userParamList)) {
                return Collections.emptyList();
            }
    ...
    private List<RelationInfo> getUserRelation(String id, String idType, String relationTypeCode, String eparchyCode, String provinceCode) {
        RequestEntity requestEntity = new RequestEntity();
        String[] param = new String[]{provinceCode, eparchyCode};
        requestEntity.setUriPaths(param);
        //组织请求参数
        ...
        if (!SysTypes.SYS_SUCCESS_CODE.equals(rsp.getRspCode())) {
            ...
            List<RelationInfo> userMemberList = JsonUtil.transMapsToObjs(rsp.getData(), RelationInfo.class);
