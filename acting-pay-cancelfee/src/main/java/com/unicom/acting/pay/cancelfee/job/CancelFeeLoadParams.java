package com.unicom.acting.pay.cancelfee.job;
 ...
 */
@Component
public class CancelFeeLoadParams {
    private static final Logger logger = LoggerFactory.getLogger(CancelFeeLoadParams.class);
    /**
     * 时间戳
     */
    ...
    /**
     * 返销相关参数每5分钟查询一次时间戳实现自动加载
     */
    @PostConstruct
    @Scheduled(fixedRate = 1000 * 60 * 5)
    private void initParam() {
        //获取系统当前时间戳
        String currTimeStamp = commParaService.getParamTimeStamp(ActingPayCommparaDef.ASM_PARAM_TIMESTAMP);
        logger.info("timeStamp = " + timeStamp + ",currTimeStamp = " + currTimeStamp);
        if ("".equals(timeStamp) || !currTimeStamp.equals(timeStamp)) {
            long startTime = System.currentTimeMillis();
            //加载返销相关参数
            writeOffRuleService.CancelFeeloadParam();
            if ("".equals(timeStamp) && "".equals(currTimeStamp)) {
                timeStamp = "1900-01-01 01:01:01";
            } else {
                timeStamp = currTimeStamp;
            }
            logger.info("获取返销参数加载一共耗时：" + (System.currentTimeMillis() - startTime) + "ms");
        }
